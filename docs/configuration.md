...
| PGSTREAM_POSTGRES_SNAPSHOT_DELTA_MAX_TABLES_PER_RUN     | 0                            | No       | Maximum number of tables that the incremental snapshot scheduler will enqueue per evaluation. `0` disables the cap.                                                                                                                                                                                          |
| PGSTREAM_POSTGRES_SNAPSHOT_DELTA_LAG_THRESHOLD_BYTES    | 0                            | No       | Minimum WAL distance (in bytes) between the current `pg_current_wal_lsn()` and the last completed table LSN before an incremental snapshot is enqueued. Set to `0` to disable the lag check.                                                                                                                 |
| PGSTREAM_POSTGRES_SNAPSHOT_DELTA_PUBLICATION_NAME       | ""                           | When delta enabled | Name of the pgoutput publication that exposes row-level changes for incremental snapshots. Required when using delta snapshots.                                                                                                                                                                              |
| PGSTREAM_POSTGRES_SNAPSHOT_DELTA_SLOT_NAME              | ""                           | No       | Optional logical replication slot name to reuse when replaying delta snapshots. Leave empty to let pgstream create and drop per-table slots automatically.                                                                                                                                                   |
| PGSTREAM_POSTGRES_SNAPSHOT_DELTA_REPLAY_TIMEOUT         | "0s"                         | No       | Maximum duration allowed for a delta replay batch. Set to `0s` to let incremental snapshots run until completion.                                                                                                                                                                                             |
| PGSTREAM_POSTGRES_SNAPSHOT_DELTA_ENQUEUE                | False                        | No       | When running `pgstream snapshot`, enqueue delta snapshot requests for the configured tables instead of copying data immediately (equivalent to `pgstream snapshot --delta`). Requires `PGSTREAM_POSTGRES_SNAPSHOT_STORE_URL`.                                                                                |
| PGSTREAM_POSTGRES_SNAPSHOT_DELTA_AUTO_PUBLICATION_ENABLED | False                      | No       | Drop and recreate the pgoutput publication automatically when delta mode is enabled. When disabled, pgstream only prints the SQL you should run manually.                                                                                                                                                |
| PGSTREAM_POSTGRES_SNAPSHOT_DELTA_AUTO_PUBLICATION_SQL   | ""                           | No       | Optional semicolon/newline separated SQL statements to execute instead of the auto-generated publication commands. Only used when `AUTO_PUBLICATION_ENABLED` is true.                                                                                                                                        |
